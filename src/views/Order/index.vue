...
import {createOrderList, getOrderInfoList} from "@/api/order";
...
const getOrderList = async () => {
  OrderInfo.value = await getOrderInfoList()
  const item = OrderInfo.value.userAddresses.find(item => item.isDefault === 0)
...
const createOrder = async () => {
  const res = await createOrderList({
    deliveryTimeType: 1,
    payType: 1,
    payChannel: 1,
    buyerMessage: '',
...
<!--              <el-button size="large" @click="addFlag = true">添加地址</el-button>-->
  ...
  <el-dialog v-model="showDialog" title="切换收货地址" width="30%" center>
    <div class="addressWrapper">
      <div class="text item" :class="{ active: activeAddress.id === item.id }" @click="switchAddress(item)" v-for="item in OrderInfo.userAddresses" :key="item.id">
        <ul>
